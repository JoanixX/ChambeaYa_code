...
from app.domain.repositories.student_repository import get_student_by_id
...
async def preprocess_all_student(session: AsyncSession = Depends(get_session)):
    ...
    try:
        processed = await preprocess_all_student(students_data)
    ...
    return {"message": "Embedding generado y guardado correctamente", 
            "total": len(students)}
