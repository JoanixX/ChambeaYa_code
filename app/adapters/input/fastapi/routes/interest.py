...
@router.post("/register/interest", response_model=Interest, tags=["Interest"])
async def register_interest(request: Request, interest: InterestCreate, session: AsyncSession = Depends(get_session)):
    try:
        ...
        result = await interest_use_case.register_interest(interest.dict())
        
        logger.info(f"Interest registrado exitosamente: {result}")
...
@router.get("/interest/all", response_model=List[dict], tags=["Interest"])
...
@router.get("/interest/{interest_id}", response_model=dict, tags=["Interest"])
